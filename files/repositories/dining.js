diningList = []; diningCategories = []; diningLocations = []; mealStyle = [];
var iter = 0;

// Tabs 2 No wrap

diningClickLink = "https://gcparksandresorts.github.io/careawayresort/things-to-do/dining/";

diningLocations = ["none", "Globe-City's Careaway Park", "Globe-City Port of Adventure", "Globe-City's Splashport Bay", "Shops & More District", "Resort Hotels"];
			// 0		1				2			3				4			5

diningCategories = ["none", "Casual Dining", "Counter Service", "Full Service", "Grab & Go", "Kiosk Ordering", "Poolside Dining"];
		//   0		1		2			3		4  	   5			6

mealStyle = ["none", "Breakfast", "Lunch", "Dinner", "Snacks & Beverages"];
	//    0		1	    2		3		4
/*
iter+=1;
diningList[iter] = "restaurantName | Description | diningLocation | diningCategory | mealStyle1,mealStyle2 | clickLink | imageLink";
*/

diningList[iter] = "Careaway Proper | Endulge in classic minecraftian eats and lay-back into the relaxing and comforting atmosphere of vacation | 1 | 3 | 1,2,3 | clickLink | https://media.nbclosangeles.com/2023/07/disneylandjuly26.jpg?quality=85&strip=all&resize=1200%2C675";

iter+=1;
diningList[iter] = "Character Cafe | Description | 1 | 2 | 1,2 | clickLink | imageLink";

iter+=1;
diningList[iter] = "Cosmic Cantina | Take a break and fuel up with some of the best grub in the galaxy. | 1 | 2 | 1,2 | clickLink | imageLink";

iter+=1;
diningList[iter] = "Yetti Yum | oo yummy | 2 | 4 | 2,4 | clickLink | imageLink";

